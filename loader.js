...
(async function() {
  try {
    ...
    // === HERO ===
    // viewport에 따라 PC/모바일 영상 자동 선택 (3:4 모바일 / 16:9 PC)
    ...
    if (videoWrap) {
      ...
      const chosenVideoId = (isMobile && c.hero.vimeoIdMobile)
        ? c.hero.vimeoIdMobile
        : c.hero.vimeoId;
      const chosenAspect = (isMobile && c.hero.vimeoIdMobile)
        ? '27/40'  // 모바일 세로 (148.15% padding)
        : '16/9';   // PC 가로
      ...
      if (chosenVideoId) {
        ...
        if (!existingVideo) {
          ...
          iframe.src = `https://player.vimeo.com/video/${chosenVideoId}?background=1&autoplay=1&loop=1&muted=1&byline=0&title=0&portrait=0&controls=0&badge=0&autopause=0&player_id=0&app_id=58479`;
          ...
          videoWrap.setAttribute('data-video-aspect', chosenAspect);
